...
void enableRawMode()
{
  ...
  IXON comes from <termios.h>. The I stands for “input flag”
   (which it is, unlike the other I flags we’ve seen so far)
   and XON comes from the names of the two control characters that
   Ctrl-S and Ctrl-Q produce: XOFF to pause transmission and XON to resume transmission.
...
The VMIN value sets the minimum number of bytes of input needed before read() can return.
 ...
 If read() times out, it will return 0, which makes sense because its usual return
